...
parser.add_argument("-r", "--reference-image", help="Reference image",
                    type=str)
parser.add_argument("--target-image", help="Reference image",
                    type=str)
parser.add_argument("-t", "--translation", help="Reference image",
                    action="store_true")
...
input = io.imread(args.reference_image)
preds = fa.get_landmarks(input)[-1]

target_image = io.imread(args.target_image)
preds_target = fa.get_landmarks(target_image)[-1]

if args.translation:
    mean_ref = np.mean(preds, axis=0)
    mean_target = np.mean(preds_target, axis=0)
    print(mean_ref)
    print(mean_target)
    translation = mean_target - mean_ref
    print(translation)
